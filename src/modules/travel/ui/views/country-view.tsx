...
import { CityAlbumCover } from "@/modules/travel/ui/components/city-album-cover";
import {
  localizeCountryName,
  localizePlaceName,
  useSiteLocale,
  ...
  return (
    <section className={styles.page}>
      ...
      <div className={styles.countryGalleryHead}>
        <p>{copy.country.cityAlbums}</p>
      </div>

      <div className={styles.countryGalleryGrid}>
        {country.cities.map((city, cityIndex) => {
          const cityName = localizePlaceName(city.city, locale);
          const href = city.id.startsWith("fallback-")
            ? fallbackHref
            : `/places/${country.code.toLowerCase()}/${toPlaceSlug(city.city)}`;
          ...
          return (
            <Link
              ...
              data-motion-hover
              aria-label={copy.country.openCityAlbum(cityName, city.photoCount)}
              key={city.id}
            >
              <div className={styles.countryGalleryImage}>
                <CityAlbumCover
                  ...
                  urls={[
                    city.image.url,
                    ...(city.fallbackImages ?? []).map((image) => image.url),
                  ]}
                  cityName={cityName}
                  priority={cityIndex < 3}
                />
